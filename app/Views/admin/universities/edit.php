...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="px-10 py-8 border-b border-slate-100 bg-slate-50/50">
            <h2 class="text-2xl font-bold text-slate-800">Edit University</h2>
            <p class="text-slate-500 font-medium mt-1">Update university details</p>
        </div>

        <form action="<?= base_url('admin/universities/update/' . $university['id']) ?>" method="POST"
            enctype="multipart/form-data" x-data="universityForm()">
            <?= csrf_field() ?>
            <?= honeypot_field() ?>
            <div class="p-10 space-y-8">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="name" id="uniName" value="<?= esc($university['name']) ?>"
                        ...
                        <div>
                            ...
                            <select name="state_id" id="stateSelect"
                                ...
                                <?php foreach ($states as $s): ?>
                                    <option value="<?= $s['id'] ?>" <?= $s['id'] == $university['state_id'] ? 'selected' : '' ?>>
                                        <?= $s['name'] ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div>
                            ...
                            <select name="type"
                                class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all font-medium"
                                required>
                                <option value="public" <?= $university['type'] == 'public' ? 'selected' : '' ?>>Public
                                </option>
                                <option value="private" <?= $university['type'] == 'private' ? 'selected' : '' ?>>Private
                                </option>
                            </select>
                        </div>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Website
                                URL</label>
                            <input type="url" name="website" value="<?= esc($university['website']) ?>"
                                class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all font-medium">
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="number" name="ranking" value="<?= esc($university['ranking']) ?>"
                                class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all font-medium">
                        </div>
                        <div>
                            ...
                            <input type="number" step="0.01" name="tuition_fee_min"
                                value="<?= esc($university['tuition_fee_min']) ?>"
                                class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all font-medium">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Tuition
                            ...
                            <input type="number" step="0.01" name="tuition_fee_max"
                                value="<?= esc($university['tuition_fee_max']) ?>"
                                class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all font-medium">
                        </div>
                    </div>
                </div>
                        ...
                        <?php if (!empty($images)): ?>
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                                <?php foreach ($images as $img): ?>
                                    <div class="relative aspect-video rounded-xl overflow-hidden border border-slate-200 group"
                                        id="img-<?= $img['id'] ?>">
                                        <img src="<?= base_url($img['file_name']) ?>" class="w-full h-full object-cover">
                                        <?php if ($img['is_main']): ?>
                                            <div
                                        ...
                                        <button type="button" @click="deleteExistingImage(<?= $img['id'] ?>)"
                                            class="absolute top-2 right-2 size-6 rounded-lg bg-rose-500 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all shadow-sm">
                ...
                <!-- Classification -->
                <div class="col-span-2 space-y-4 mt-6">
                    <div class="flex items-center gap-6 p-4 bg-slate-50 rounded-xl">
                        <div class="flex items-center gap-3">
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="stem_available" id="stemToggle" value="1"
                                    class="sr-only peer" <?= $university['stem_available'] ? 'checked' : '' ?>
                        ...
                        <div id="classificationWrap"
                            class="<?= $university['stem_available'] ? 'hidden' : 'flex' ?> items-center gap-3 border-l border-slate-200 pl-6">
                            <span
                            ...
                            <div class="flex gap-2">
                                <label class="flex items-center gap-2 cursor-pointer group">
                                    <input type="radio" name="classification" value="International" class="hidden peer"
                                        <?= $university['classification'] == 'International' ? 'checked' : '' ?>>
                                    <div
                                ...
                                <label class="flex items-center gap-2 cursor-pointer group">
                                    <input type="radio" name="classification" value="Domestic" class="hidden peer"
                                        <?= $university['classification'] == 'Domestic' ? 'checked' : '' ?>>
                                    <div
                                        class="px-3 py-1.5 rounded-lg border border-slate-200 text-xs font-bold text-slate-500 peer-checked:border-indigo-600 peer-checked:bg-indigo-50 peer-checked:text-indigo-600 transition-all hover:bg-slate-100">
                                        Domestic</div>
                                </label>
                ...
                <div class="col-span-2 pt-4" x-show="requirements.length > 0 || countryInfo" x-cloak>
                    ...
                    <div
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="metadata[about]" id="aboutField" rows="2" x-model="savedMetadata.about"
                                class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 transition-all font-medium"><?= esc($university['metadata']['about'] ?? '') ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function universityForm() {
        return {
            countryId: <?= json_encode((string) $university['country_id']) ?>,
            savedMetadata: <?= json_encode($university['metadata'] ?? []) ?>,
            requirements: [],
            countryInfo: null,
            loading: false,
            init() {
            ...
            async deleteExistingImage(imageId) {
                ...
                try {
                    ...
                    const response = await fetch(`<?= base_url('admin/universities/delete-image') ?>/${imageId}`, {
                        method: 'POST',
                        body: formData
                    });
                    const data = await response.json();
    ...
    function toggleClassification() {
        const stem = document.getElementById('stemToggle').checked;
        const wrap = document.getElementById('classificationWrap');
        if (!stem) {
